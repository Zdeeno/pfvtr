...
class CNNOLD(t.nn.Module):
    ...
    def __init__(self, lp, fs, ech, res, legacy=False):
        ...
        if self.legacy:
            self.backbone = t.nn.Sequential(*[create_conv_block(3, 16, 3, 1, 1, (2, 2), pool_layer=False),
                                              ...
                                              create_conv_block(512, 256, 3, 1, 1, (3, 1), bn=False, relu=False, pool_layer=False)])
        else:
...
def load_model(model, path, optimizer=None):
    checkpoint = t.load(path, map_location=t.device("cpu"), weights_only=False)
